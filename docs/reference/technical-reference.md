...
- **Boots are INSIDE the sweep as of slice 5, but stay tag-based.** They are still not a wiki `itemlimit` group (see above), so the rule cannot ride the mutex machinery: a name counts as boots when ANY same-named catalog variant carries the `Boots` tag (the mode rule's any-variant quantifier, for the same ID-partition reason). An owned pair seeds the boots slot before the sweep (`boots-collision` drops then say "player owns X"), the first path boots entry wins otherwise, and the rule-2 owned-echo exemption applies as usual. The standalone `findDuplicateBoots` detect-and-warn helper is deleted; the sweep owns the rule.
