...
- **Optional fields must be nullable.** Use `type: ["string", "null"]` (or `["array", "null"]`, etc.) and list the property in `required`. The model then returns `null` instead of omitting the field.
