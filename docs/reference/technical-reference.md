...
`enforceBuildPathLegality(buildPath, items, mode, ownedItemNames)` folds four rules into one first-entry-wins pass: no name twice in the path, no entry that no same-named catalog item makes `isBuildPathEligible` in the current mode (dropped as `mode-unavailable`, the deterministic backstop for cross-mode leaks when the schema enum is degraded or the `recommendations[]` fallback was promoted), no two different items sharing a mutex group, and no entry the shop would refuse given the player's CURRENT inventory (owned items seed the group-occupancy map before the sweep). Non-obvious findings:
...
- Boots remain OUTSIDE this sweep: they are not a wiki `itemlimit` group (see above), so the tag-based `findDuplicateBoots` warn path from #109 still owns that rule.
