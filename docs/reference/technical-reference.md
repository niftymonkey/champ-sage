...
- A trusted rendering REPLACES `champion.abilities.passive.description` at ingest-merge (`mergeAbilityScaling`) before the cache write; `CACHE_VERSION` bumped to 8 so existing installs re-ingest. Innate quarantine reasons share the audit histogram (example label "innate") but never touch the spell-stat counters.
