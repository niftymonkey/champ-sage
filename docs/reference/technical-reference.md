...
**The wiki is the description bottleneck.** `cherry-augments.json` carries no descriptions (only `id`, `nameTRA`, icon, `rarity`); readable text comes from the wiki Lua module, which tracks live and is NOT PBE-versioned. So PBE-only augments arrive from cdragon with IDs/icons but no wiki match, and the merge drops them (no description to coach on). This degrades silently, it does not crash. Full readiness tracks wiki coverage, which we do not control: re-check as the wiki catches up.
...
Two distinct Overwolf-side failures can silently kill augment coaching while everything LCU-sourced (enemy list, your items, gold, item recs) keeps working, because those ride Live Client Data / LCU polling, not GEP:
...
The override workaround (`scripts/ow-package-guard.ts`, fed to ow-electron via the supported `--owepm-packages-url` flag) compensates only while the manifest is down. Pinning an absolute version is a treadmill: it goes stale on every floor bump, and Overwolf rotates old builds off the CDN (older versions return `403 AccessDenied`; only the most recent few are live). So the guard auto-discovers the newest live GEP by probing the CDN (no listing or `latest` alias exists, ranged `GET bytes=0-0` returns 200/206 live vs 403 rotated), keeps overlay/utility on compat-stable pins that self-heal if rotated, and reconciles the local cache (OWEPM loads GEP from its on-disk cache and only re-downloads when that copy is gone or its version changes, so a stale cached build must be purged for a new override version to take effect). The permanent fix is Overwolf's manifest API recovering, at which point the override is deleted and normal resolution resumes.
