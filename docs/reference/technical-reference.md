...
- Level, current gold
- Full rune page: keystone, all 6 general runes, primary/secondary trees. **In ARAM Mayhem (KIWI) the rune fields come back empty**: runes are not a thing in that mode (observed across multiple live games, 2026-07-18), so `buildBaseContext` omits the `Runes:` line for Mayhem rather than rendering `Runes:  ( / )`. Regular ARAM does have runes; an empty rune page there would be a real parse/API bug worth investigating.
...
**Gotcha: DDragon's `maps` field is reliable for "on no map" but INCOMPLETE for "on the ARAM map."** Each item carries `maps: { "11": bool, "12": bool, "30": bool, ... }` (11 = Summoner's Rift, 12 = Howling Abyss/ARAM, 21 = Nexus Blitz, 30 = Arena; a new id 453 appears alongside 12 on some ARAM-available items as of 16.x). Ingest parses the true entries into `Item.maps` (number[]). It is tempting to filter the ARAM catalog to `maps` includes 12, but that DROPS REAL STAPLES: verified 2026-07-17, Guardian Angel (3026), Mejai's Soulstealer (3041), and Bastionbreaker all carry `map12 = false` yet are genuinely buildable in ARAM. So a literal map-12 filter is a regression. What `maps` IS reliable for is exclusion: an item available on NO map (`maps` is `[]`) is deprecated or internal (Deprecated item 3095, Quest markers, Guardian's Amulet/Shroud) and recommendable nowhere, and an item whose maps DON'T intersect a mode's allowed set is a cross-mode leak. `isBuildPathEligible` therefore ANDs two checks (#117 mode-availability slice): the permissive ID-range partition (so staples survive) and a **maps-intersection rule** against per-mode allowed sets defined next to `modeAcceptsItemMode` in `item-catalog.ts`: Classic {11}, ARAM/Mayhem {11, 12, 35} (11 kept because of the map12 false negatives; 35 observed on ARAM-flagged items like the Hubris variant [12, 35] and unmodeled, so permissive), Arena {30}. The intersection closes the standard-ID-band leaks the partition cannot see: Arena-only maps [30] items (Rite Of Ruin 3430, Perplexity 4015, Wordless Promise 4016, Hellfire Hatchet 4017, ...) and Nexus-Blitz-only maps [21] items (Ghostcrawlers 3005, Deathfire Grasp 3128, Sword of the Divine 3131, Innervating Locket 4402), plus Classic-only leaks like Atma's Reckoning 3039 [12, 21]. Watch for variant pairs when auditing: Bloodletter's Curse 4010 is Arena-only [30] and correctly drops, while 8010 (same name, the live SR item) carries [11, 12, 21, 35, 453] and correctly stays. An empty-maps rejection falls out of the intersection for free. `pnpm check-buildpath-enum` carries leak canaries (Perplexity, Hellfire Hatchet, Deathfire Grasp, Ghostcrawlers must be absent from every non-Arena enum). A handful of DDragon-flagged-ARAM meme/starter items (The Golden Spatula, the 950g `Guardian's` Lane starters) carry `map12 = true` and no clean structural signal removes them without also dropping a real item (the `Lane` tag catches Atma's Reckoning too). The Golden Spatula is especially deceptive: DDragon marks it `purchasable: true`, but in real games it is granted by a rare augment and cannot be bought. These are handled by `NEVER_RECOMMEND_ITEM_NAMES`, a small curated name blocklist inside `isBuildPathEligible` (name-based so it survives id churn and catches all same-named variants, e.g. Golden Spatula ids 4403/224403/994403). Fully map-accurate availability remains issue #138.

**Gotcha: same-named item variants must be deduped for the catalog.** DDragon ships multiple purchasable items sharing a display name (e.g. Abyssal Mask `8020` at 2650g on SR+ARAM, and `328020` at 2850g), and the permissive mode filter passes both. The base context tells the model each catalog name is a distinct purchasable item, so two "Abyssal Mask" lines at different prices is actively misleading. `buildItemCatalogSections` collapses tier 2 by display name via `dedupeByName`, keeping the most broadly-available variant (most maps, then cheapest, then lowest id). A real Kog'Maw ARAM prompt went from 20 duplicated names to 0.

**This trap was documented here and the coaching item catalog fell into it anyway (fixed, #145).** `buildItemCatalogSections` used `filterItemsByMode` for its tier-2 "Other available items" reference, so in ARAM/Mayhem the model was shown ~26 items and simultaneously told by the base context that "if a name does not appear there, it is NOT a purchasable item". Meanwhile the game-plan `buildPath` enum, in the same file, offered 142 items via `isBuildPathEligible`: two features disagreeing about which items exist. Tier 2 now derives from `isBuildPathEligible` too, so the catalog the model reads and the enum it must answer within are the same set by construction (Jhin ARAM: tier 2 went 26 to 154 items, prompt 7.1KB to 12.5KB). Tier 1 (the meta/presence pool) had the mirror-image bug until the #117 mode-availability slice: it filtered pool entries with ad-hoc structural checks (gold floor, `into`) and never consulted eligibility, so blocklisted starters (Guardian's Horn/Hammer, which genuinely sit in the ARAM presence pools) and cross-mode leaks rendered in the "Item pool for <champ>" section while the enum excluded them. Tier 1 now gates every resolved pool entry through the same `isBuildPathEligible(item, mode)` call, making it a subset of the enum by construction; the ad-hoc checks were deleted as redundant (the predicate covers all of them). `selectItemMode` was deleted with it; it existed only to feed `filterItemsByMode` here and was a live re-entry point for the bug. **`ModeContext.modeItems` (`src/lib/mode/{aram,aram-mayhem,classic}.ts`) still calls `filterItemsByMode` and still carries this bug, but is currently write-only (nothing reads it).** Fix or delete it before anything consumes it.
...
- Ingest: `wiki-item-groups.ts` fetches and parses it (reusing `parseLuaTable`), `mergeItemMutexGroups` attaches `Item.mutexGroups` by lowercase name (so same-named mode variants inherit it) before the cache write (cache v8). Enforcement: the item catalog renders a "Purchase restrictions" section for any group with 2+ pool members, and `enforceBuildPathLegality` (game-plan module; evolved from the earlier `enforceMutexGroups`) deterministically drops later same-group entries from returned build paths (first entry wins). `pnpm check-item-mutex-groups` verifies the live wiki format hasn't drifted.

### Build-path legality sweep is inventory-aware and must honor the end-state contract (#117 slice 3)

`enforceBuildPathLegality(buildPath, items, mode, ownedItemNames)` folds five rules into one first-entry-wins pass: no name twice in the path, no entry that no same-named catalog item makes `isBuildPathEligible` in the current mode (dropped as `mode-unavailable`, the deterministic backstop for cross-mode leaks when the schema enum is degraded or the `recommendations[]` fallback was promoted), at most one Boots-tagged item (#109, folded in by slice 5), no two different items sharing a mutex group, and no entry the shop would refuse given the player's CURRENT inventory (owned items seed the group-occupancy map and the boots slot before the sweep). Non-obvious findings:

- **Owned items must NOT be dropped from the path.** `buildGamePlanQuestion()` defines `buildPath` as the 6-slot END-STATE inventory and requires owned items to reappear in it, so an owned name's first path appearance is the expected echo and only a SECOND appearance is a duplicate. A naive "already owned = drop" rule would gut every mid-game plan and contradict the prompt.
- **The inventory source is the game snapshot, by design.** `snapshot.player.items` (built by `takeGameSnapshot` from `liveGameState.players[isActivePlayer].items`) is the exact list the `[Game State] Items:` prompt line renders, so validator and prompt agree. It carries NAMES only (ids are dropped at snapshot build), and it includes everything the live client reports: components, consumables, potions, trinkets, wards. That is harmless for the sweep (those names never match completed-item path entries and carry no `itemlimit` groups).
- **Owned evolved items count as their purchasable base.** The player holds "Muramana" but the catalog enum can only echo "Manamune"; the sweep resolves owned names through the exported `resolveToPurchasable` walk (`item-catalog.ts`) and lets the BASE name occupy the name slot, while the RAW owned name holds the group slot so remediation logs say "player owns Muramana", the string the player actually sees.
- **Unknown owned names are inert by construction**: they only block an exact same-name re-buy (name equality needs no catalog proof) and can never occupy a group, so the sweep drops nothing it cannot prove illegal. The mode rule follows the same principle: unknown path names get no mode verdict.
- **The mode rule quantifies over ALL same-named variants** (an entry is legal if ANY catalog item with that name is eligible). Duplicate names span ID partitions (Abyssal Mask is standard 8020 AND aram-band 328020, Bloodletter's Curse is Arena-only 4010 AND SR 8010), so judging by one arbitrary variant would false-drop real items. Owned-item echoes are exempt from mode drops entirely: what the player holds is a fact of the game, not a recommendation.
- **Boots are INSIDE the sweep as of slice 5, but stay tag-based.** They are still not a wiki `itemlimit` group (see above), so the rule cannot ride the mutex machinery: a name counts as boots when ANY same-named catalog variant is both `Boots`-tagged and eligible (the mode rule's any-variant quantifier, for the same ID-partition reason; see the next bullet for why eligibility is part of the test). An owned pair seeds the boots slot before the sweep (`boots-collision` drops then say "player owns X"), the first path boots entry wins otherwise, and the rule-2 owned-echo exemption applies as usual. The standalone `findDuplicateBoots` detect-and-warn helper is deleted; the sweep owns the rule.
- **The `Boots` tag alone does not mean "occupies the boots slot": DDragon's tier-1 `Boots` (id 1001, 300g) carries it.** Verified live: `{"name":"Boots","tags":["Boots"],"gold":{"total":300}}`. Every boots build passes through that component, so a tag-only rule reads a mid-upgrade inventory as "already has boots" and false-drops the tier-2 pair the player is saving for, exactly when early-game item advice matters most. The boots-slot test is therefore tag AND `isBuildPathEligible` (whose sub-500g floor excludes the component). The eligibility half is redundant for path entries, since the mode rule already dropped anything with no eligible variant before the boots check runs, but it is load-bearing for the inventory, which bypasses the mode rule entirely.

### Corrective retry: one shot, history-replacing, swept path always ships (#117 slice 5)

When the sweep drops anything, the game plan is legal but short (a dropped slot is a lost recommendation). `remediateGamePlan` (`src/lib/ai/features/game-plan/remediation.ts`) gives the model exactly ONE chance to reclaim those slots, and its design encodes three hard-won constraints:

- **The corrective message must restate the offending path.** Game-plan's `summarizeForHistory` stores only `result.answer` as the assistant turn, so the illegal `buildPath` is NOT in conversation history; a bare "fix your violations" correction would reference items the model never "said". `buildCorrectiveMessage` restates the full path by name and states each broken rule in the task prompt's own vocabulary (restriction group names, "at most one Boots item", "not purchasable in \<mode\>", "listed twice", "already own"). It is model-facing prose, deliberately distinct from `describeBuildPathDrop`'s log fragments.
- **The retry goes through `MatchSession.correctLastAsk`, which is history-REPLACING.** It requires the last history message to be the assistant turn of a prior `ask` for the same feature (throws otherwise; any manual history mutation invalidates it), sends `[...history, transient correction turn]` under the same composed system prompt as `ask`, and on success swaps the last assistant turn's content for the corrected `summarizeForHistory`. The correction turn is never persisted: the corrective exchange must not leak into later voice-query context, and the corrected answer supersedes the first. History ends as [user: original question, assistant: corrected answer]. On failure history is untouched, so falling back to the first response is safe.
- **The swept (legal) path ALWAYS ships.** Exactly one corrective call ever: a clean retry ships re-swept; a still-violating retry ships its swept remainder (no second correction); a thrown/aborted corrective call falls back to the first response's swept path. This deliberately diverges from the issue #117 text ("ship the 6 items" on a failed retry): the established sweep position (slice 3) is that a shorter legal path beats an illegal 6-item path. `CoachingPipeline.submitGamePlanQuery` logs drops with a corrected/retry-failed marker and pushes the returned `answer` + swept `buildPath` to the feed and overlay.

Eval side: the `Boots Uniqueness` gate scorer is replaced by `Item Legality`, which runs the full `enforceBuildPathLegality` sweep (all five rules, inventory-seeded from `scorerContext.items`, mode resolved from the fixture's `gameModeId`) and scores 1 iff nothing drops. It deliberately measures the RAW first response, not post-remediation output: the gate tracks the MODEL's violation rate while production guarantees legality regardless, and a retry inside evals would double per-case cost. `fixtures/coaching-sessions-v2/synthetic-game-plan.json` ("update the game plan", Classic mid-game with owned items) is the first fixture that routes to game-plan, so the game-plan scorers are no longer inert 1.0 baselines. `scripts/inspect-evals.ts` had drifted from `coaching.eval.ts` (it still listed the deleted "Augment Re-Roll Accuracy" gate and never gated on boots); its `GATE_SCORERS` now matches again. Keep the two lists in sync by hand; they are string literals against the evalite SQLite cache.

### Build paths and recommendations need different legality rules (#117 item-rec slice)

Item-rec has its own deterministic rail (`enforceRecommendationLegality` + `remediateItemRec`, `src/lib/ai/features/item-rec/`), and copying the build-path sweep into it would be wrong. The two surfaces mean different things, and the rules follow from the meaning:

- **A build path is an END-STATE inventory; a recommendation list is a set of ALTERNATIVES.** The player buys every build-path entry and exactly one of the offered options. So options do NOT eliminate each other: two Last Whisper items, or two pairs of boots, side by side are a legitimate comparison ("Mortal Reminder for the healing matchup, Lord Dominik's for the tank"), while the same pair inside one build path is illegal. Only a slot the player's INVENTORY holds eliminates an option. The build path's first-entry-wins behavior is inverted here, deliberately.
- **The echo rule inverts too.** `buildGamePlanQuestion()` REQUIRES owned items to reappear once in the build path, so the sweep keeps an owned name's first appearance. An owned item as a recommendation is never legal: buying it again buys nothing. Owned matching covers both the display name and its purchasable base through `resolveToPurchasable`, so owning Muramana kills a Manamune option (`already-owned`, with the drop naming "Muramana", the string the player actually sees).
- **Mode availability and unknown names are identical across both.** An option is mode-legal when SOME same-named catalog variant is `isBuildPathEligible`, and a name the catalog does not know is never dropped: nothing about it is proven.
- **Shared machinery lives in `src/lib/ai/item-legality.ts`**, not in either feature. `buildItemNameIndex(items, mode)` collapses the id-keyed catalog into name-keyed facts (mutex groups unioned across same-named variants, one representative item per name, mode-legal names, boots names), and `resolveInventorySlots(index, items, ownedItemNames)` resolves what the player already occupies (owned names including evolved bases, group holders, the boots slot), keying every value to the OWNED display name. Both the sweep and the option filter stand on those two; the name-index construction must not be re-derived per feature.
- **The corrective retry mirrors game-plan's**, for the same reason it exists there: the answer prose names the option cards, so silently deleting a card leaves the two disagreeing. One `correctLastAsk` turn naming each violation in the task prompt's vocabulary, then filter the corrected response too, and ALWAYS ship the filtered result (a failed corrective call warn-logs and ships the first response's filtered options). Wiring covers BOTH item-rec call sites in `CoachingPipeline` (the voice route and the proactive shop-moment / gold-available triggers) through one `applyItemRecLegality` helper, so the feed and the overlay payload always carry the same filtered list.

Motivating playtest (2026-07-29, ARAM Mayhem): the item-rec window offered Mortal Reminder while the player held Lord Dominik's Regards. The item catalog's "## Purchase restrictions" section was already in the prompt and the model slipped anyway, which is the standing argument for a deterministic rail behind every prompt rule about cross-item legality.

### Transformation-only items: `purchasable === false` + `specialRecipe` (verified 2026-07-28)

Some items can never be bought: they enter inventory by transforming a base item (Manamune becomes Muramana at 360 mana stacks). Recommending one as a purchase ("Muramana ... Cost: 2900g") is nonsense the model cannot recover from. What the data actually says:

- **Detection signal is `item.gold.purchasable === false` in DDragon.** `inStore === false` (DDragon only emits it with value `false`) and CommunityDragon's `inStore` are ID-for-ID coextensive with it; the three signals are one, so do not add a second. The real transformation-only set is FIVE items: Muramana, Seraph's Embrace, Fimbulwinter, Runic Compass, Bounty of Worlds, plus mode-variant ID clones carrying identical flags. Vigilant Wardstone and the Bounty of Worlds "choice of 5 Legendaries" finals are purchasable-but-gated shop transactions, NOT transforms; they need gating logic, not this filter.
- **The transform-to-base pointer is `specialRecipe`** (the transform carries its base item's DDragon id), NOT the `from` array (empty on transforms). Two caveats: `purchasable` is the gate and `specialRecipe` only the pointer, because one purchasable item also carries it (Arena Prowler's Claw 446693 pointing at an Anvil Voucher); and clones can point cross-partition (Arena Fimbulwinter 223121 points at standard 3119, not 223119), so any walk must resolve by id in the FULL item map. Bounty of Worlds needs two hops (via Runic Compass) to reach purchasable World Atlas.
- **Community meta pools contain ONLY the evolved forms** (they are end-of-game inventories, the only form players finish with): live `aram.json` had Seraph's Embrace in 13 champion pools, Muramana in 13, Fimbulwinter in 23, and the base items in ZERO pools. So dropping transforms without substitution deletes the whole mana-item line from tier 1. `buildItemCatalogSections` resolves each pool entry via `resolveToPurchasable` (walk `specialRecipe` until purchasable) and, when base and transform independently clear the presence floor, SUMS their presences (each game's end inventory holds exactly one of the two, so the sum is not a double count). Tier 2 and the game-plan enum were already safe via the `!purchasable` check in `isBuildPathEligible`.
- **Transforms stay in the ingested item map** (they resolve inventory display when the player holds the evolved form); the exclusion is about what gets RECOMMENDED. Ingest carries `specialRecipe` into `Item` (cache v9).
- **Support-line description bugs in DDragon:** World Atlas has an EMPTY description, and Runic Compass's description contains an unsubstituted placeholder ("Earn 0 gold from this item..."), so support-quest trigger text is not cleanly derivable from DDragon. The mana line is fine: each base item's description carries its trigger sentence ("Transforms into Muramana at 360 max Mana."), which self-documents the substitution in tier-1 prompts.
...
- **50 titles per request** is the unauthenticated cap. The full roster is 173 champions x 5 titles (innate + Q/W/E/R) = 865 titles = 18 requests, fetched sequentially to stay a good citizen.
...
- `unknown-template:ccd` (4): champion crit-data lookups (Yasuo/Sivir crit rows). `pplevel` and `times` were taught during the innate work and no longer quarantine.
...
**Ingest wiring:** scaling is merged onto `champion.abilities.spells[i].scaling` BEFORE `writeCache` (same rule as abilities themselves), keyed by lowercase champion NAME (the champions map key), matched to slots positionally in Q/W/E/R order. The fetch is chained off the champion fetch but kept inside the same `Promise.all` so its ~18 round-trips overlap the other sources. Unlike abilities, low scaling coverage NEVER blocks the cache write (`SCALING_MIN_COVERAGE_RATE` only warns): scaling is an enhancement, so a wiki outage should cost prompts their damage numbers for a session, not cost the app its game data.
...
### Innate (passive) descriptions from the wiki /I pages (implemented 2026-07-18)

DDragon's passive text is numberless flavor prose, yet passives are what augments most often synergize with. `Template:Data {Champion}/I` redirects to the innate's named page (same `Ability data` template, `skill = I`) and rides in the same 50-title batches. Measured on 16.14.1: **155/173 innates (89.6%) render**; the rest fall back to DDragon text. Coverage and reasons: `pnpm audit-ability-scaling`.

- **Innate pages have NO `leveling` params.** Their numbers live in `description`/`description2`/... params via `{{pplevel|26 to 196}}` (renders "26 to 196 (based on level)") or prose. So they render as prose through `stripWikiMarkup`, staged like the stat parser's `resolvePart`: variable substitution, arithmetic evaluation, parser-function refusal, unknown-template refusal, then a residual gate (surviving `''` quote runs or brackets quarantine the whole passive back to DDragon text).
- `{{pplevel|key=%|20 to 30}}` declares a percent unit; renders "20 to 30% (based on level)". Arithmetic forms stay verbatim ("26*0.4 to 196*0.4"): unambiguous to an LLM, no evaluator applied.
- **Icon-family templates dominate innate prose:** `{{sti}}`/`{{stil}}`/`{{ci}}`/`{{cci}}` render their LAST param (earlier params are icon/link keys), `{{cis|X}}`/`{{ais|X|Champ}}` are possessives ("X's"), `{{ccs|display|type}}` keeps its FIRST param, `{{rutngt|0.01}}` keeps the value, `{{times}}` is a bare multiplication sign, `{{bug|...}}` is editorial and drops. Teaching `sti` alone recovered 92 innates.
- **The bold/italic strip regexes could not cross an apostrophe** (`'''Braum's'''` survived as literal quote runs) until switched to lazy `[\s\S]*?` matching. `{{ft|...}}` is written ATTACHED to adjacent words in wiki source (`take{{ft|...}}from`), so its output is space-padded and the final whitespace collapse absorbs the doubles.
- The 18 remaining fallbacks are crit-math (`{{ccd}}`, `{{critical damage}}`) and layout (`{{column}}`) pages (Ashe, Caitlyn, Yasuo, Nocturne, ...). Expected: evaluating crit formulas confidently is not possible, and DDragon text still describes those passives.
- A trusted rendering REPLACES `champion.abilities.passive.description` at ingest-merge (`mergeAbilityScaling`) before the cache write; `CACHE_VERSION` bumped so existing installs re-ingest. Innate quarantine reasons share the audit histogram (example label "innate") but never touch the spell-stat counters.
- **`{{pp}}` values can carry a growth-step count that is presentation, not value.** Braum's Concussive Blows writes the stun duration as `{{pp|changedisplay=true|1.25 to 1.75 for 3|1 to 13|...}}`: the trailing "for 3" is how many steps the value grows in. Kept verbatim mid-sentence it rendered "stun them for 1.25 to 1.75 for 3 seconds". `stripGrowthSteps` drops a trailing `for N` only, so ordinary ranges and values that merely end in a number survive. This class of defect is invisible to coverage metrics: the page renders, quarantines nothing, and the audit counts it as a success. **`pnpm dump-wiki-innate <Champion>` prints the source params beside the rendered sentence**, which is the only practical way to find the next one.

### Scaling profile line and STAT SYNERGY rule (implemented 2026-07-18)

`src/lib/ai/scaling-profile.ts` derives a deterministic "Scaling profile:" summary appended as the last line of the Abilities block, parsing ONLY the strict `(+ N% STAT)` token form (a lenient parse would fold shapes like "(+ 4% per 100 AP)" into a flat ratio and misstate the kit). Max ratio per stat per ability; "up to" marks several ratios collapsed; "(shield)"/"(heal)" mark defensive ratios; "No AP ratios."/"No AD ratios." state absent coverage explicitly because those are the stats augments most commonly grant. The augment-fit task prompt carries a STAT SYNERGY rule with a cite-the-ratio clause, so suspect fit grades show in their reasoning whether the ratios were consulted. The prompt-summary log field `scalingRatios` is renamed `scalingRatioTokens`: it is a verification count scanned from the rendered prompt, not the data the model sees.
...
**Summoner-spell slot order is a keybind, not a pairing.** Match-v5 participants carry `summoner1Id`/`summoner2Id` (4 = Flash, 6 = Ghost, 7 = Heal, 11 = Smite, 12 = Teleport, 13 = Clarity, 14 = Ignite, 32 = the ARAM-default Mark/Dash snowball). The two slots only reflect the player D/F keybind, NOT a canonical pairing, so `[32,4]` and `[4,32]` are the same combo. `extractMatchData` captures them as `summonerSpells`, filtered to present/positive values so legacy cached matches (the field predates them) and remakes yield an empty pair. `aggregateBuilds` clusters per champion by the pair NORMALIZED ascending, collapsing the two slot orders into one `popularSpells` entry (most-picked first, with pick/win rates) instead of two half-counted ones, and skips any participant without a complete 2-spell pair. Sanity signal: in ARAM, spell 32 (mode-default) leads almost every pair and 13 (Clarity) appears only in ARAM, so a ranked-looking distribution in an ARAM file means the wrong queue was collected.

**Per-champion item-pool sample size (the coverage study, `pnpm analyze-coverage`).** Empirically, an item-usage pool needs FAR more games than the old `CHAMPION_PARTICIPANT_TARGET = 40` to be stable. Measured on 87k cached ARAM matches (top-K Jaccard of a subsample's most-built items vs the champion's full-data pool): the core top-6 reaches ~0.90 only around N=160, and the wider top-12/18 pool (the situational items dynamic selection draws on) never reaches 0.90 even at N=320, it keeps sharpening logarithmically. At N=40, roughly 2 of every 12 pool items are noise. Working bar: ~200 games per champion. Player diversity is NOT a concern for ARAM: champion assignment is random, so the busiest single player is a median 0.27% (max 1.45%) of any champion's games and a concentrated sample cannot even be constructed, so no distinct-player floor is needed (this would differ for pick modes like ranked/Arena). Reachability is volume-bound, not targetable: Match-v5 cannot query "games featuring champion X", and in random-assignment ARAM no player reliably produces a given champion, so the only lever is total volume. At 87k matches all 173 roster champions (the Data Dragon denominator) already have >= 200 games, including brand-new ones, so a "stop when every champion has >= N games" terminator at N ~ 200 is both achievable and terminating, unlike the current saturate-or-exhaust model. Pure metrics live in `src/lib/meta-builds/coverage-analysis.ts`.

**The item pool is sourced from item PRESENCE, not exact-set build clusters.** The product ships an item POOL, never a build order: the coaching LLM picks contextually from the items a champion's players use regularly (`src/lib/ai/item-catalog.ts` Tier 1). The original pool came from the top-10 exact-6-item-set clusters, which is statistically near-noise: exact-set clustering shatters into 2-8 game singletons no matter the volume (in a 60k-match ARAM file the top build cluster captured a median 1.3% / max 4.9% of a champion's games, and zero of 166 champions had any single cluster with >= 20 games even at 500-850 games each). `aggregateBuilds` now also emits `itemPool: {itemId, presence}[]` per champion, built via `itemPresence` (`coverage-analysis.ts`): every item built in `>= ITEM_POOL_PRESENCE_FLOOR` (0.10) of the selected games, ranked by presence descending, capped at `ITEM_POOL_MAX_SIZE` (30). A presence FLOOR (not a fixed top-N) makes pool size data-driven and handles multi-build champions for free: a single-direction champion clears ~8-12 items, a genuine two-build champion clears more because both cores pass the floor, so no separate multi-build detector is needed (the LLM disambiguates from the presence rate + item stats). The change is ADDITIVE: `deriveMetaItemPoolEntries` (`meta-builds.ts`) prefers `itemPool` and carries the rate into the prompt as "(used in N% of games)", falling back to legacy `builds` (null rate) for ranked-solo/arena files that predate the field. First full ARAM run (patch 16.13, 173 champions): pool size median 13 (range 10-18), e.g. Jinx's pool is Berserker's 85% / IE 64% / situational items lower, which reads as real itemization rather than the old noise.

**Collection readiness / stop-criterion (`src/lib/meta-builds/readiness.ts`).** The convergence study established ~200 games per champion as the presence-pool stability bar, but the collector historically stopped only on queue-exhaustion/saturation (per-champion coverage was diagnostic-only). `readiness.ts` (pure) tallies per-champion in-window games (`tallyChampionGames`) and reports progress against `COLLECTION_GAME_TARGET = 200` (`buildReadinessReport`): ready-count, rarest champion, and the laggards below target. The snowball prints a live readiness line every 1000 matches and AUTO-STOPS when every seen champion clears the target (`endReason = "all-champions-ready"`); saturation/exhaustion stays the backstop. In practice the auto-stop rarely fires for ARAM in a 30-day window because a handful of rare champions never reach 200 (first full run: 144/173 champions >= 200, median 359 games/champ, rarest RekSai 61), so the end-of-run laggard report is the practical "is the tail good enough to stop?" tool.
...
- **Example (#109):** the game plan's 6-item `buildPath` must contain at most one Boots-tagged item. The `name` enum restricts each element to a valid catalog entry but allows multiple boots (each pair is individually valid). The rule lives in `GAME_PLAN_TASK_PROMPT` as a sentence and is guaranteed after the call by the boots rule inside `enforceBuildPathLegality` (`src/lib/ai/features/game-plan/index.ts`); since #117 slice 5 the pipeline goes beyond detect-and-log to drop + one corrective retry (`remediateGamePlan`), and the standalone `findDuplicateBoots` detector is deleted.
- **Eval coverage:** `Item Legality` gate scorer in `coaching.eval.ts` runs the full `enforceBuildPathLegality` sweep on the raw model output so model-comparison runs surface the violation rate (boots, duplicates, mode availability, restriction groups, owned items) alongside other hard-correctness gates.
...
**A renderer clear needs a tombstone to reach the merge.** The registry turns `ok` into a deletion, so piping it straight into `localStatus$` leaves nothing for `mergeStatuses` to act on and the merge falls back to whatever main last said. `streams.ts` therefore wraps the registry, remembers every subsystem the renderer has ever had an opinion about, and republishes the cleared ones as explicit `{ level: "ok" }` entries: the merge lets them win for the id, then filters them out. The wrapper is what makes **clear-before-set** work, and that is the ordinary case rather than an edge one: `clear(id)` returns silently when it finds nothing to delete, and a first load that simply works clears `data` having never set it. Subscribing to the registry's own emissions would therefore miss every happy path. Without this, a main-process report the renderer has since disproved would keep its banner up forever, with nothing in the app able to take it down. Nothing overlaps today (the renderer only reports `data`), which is exactly why the gap was invisible. Tests that assert on the renderer's own status must read through `mergeStatuses`, not the raw `localStatus$`, or they see the tombstone and call it a banner.
