...
**Gotcha: DDragon's `maps` field is reliable for "on no map" but INCOMPLETE for "on the ARAM map."** Each item carries `maps: { "11": bool, "12": bool, "30": bool, ... }` (11 = Summoner's Rift, 12 = Howling Abyss/ARAM, 30 = Arena). Ingest parses the true entries into `Item.maps` (number[]). It is tempting to filter the ARAM catalog to `maps` includes 12, but that DROPS REAL STAPLES: verified 2026-07-17, Guardian Angel (3026), Mejai's Soulstealer (3041), and Bastionbreaker all carry `map12 = false` yet are genuinely buildable in ARAM. So a map-12 filter is a regression. What `maps` IS reliable for is the empty case: an item available on NO map (`maps` is `[]`) is deprecated or internal (Deprecated item 3095, Quest markers, Guardian's Amulet/Shroud, all-false Arena leftovers like Deathfire Grasp) and recommendable nowhere. `isBuildPathEligible` therefore keeps the permissive ID-range mode filter (so staples survive) and only ADDS a `maps.length === 0` rejection for junk. A handful of DDragon-flagged-ARAM meme/starter items (The Golden Spatula, the 950g `Guardian's` Lane starters) carry `map12 = true` and no clean structural signal removes them without also dropping a real item (the `Lane` tag catches Atma's Reckoning too). The Golden Spatula is especially deceptive: DDragon marks it `purchasable: true`, but in real games it is granted by a rare augment and cannot be bought. These are handled by `NEVER_RECOMMEND_ITEM_NAMES`, a small curated name blocklist inside `isBuildPathEligible` (name-based so it survives id churn and catches all same-named variants, e.g. Golden Spatula ids 4403/224403/994403). Fully map-accurate availability remains issue #138.
...
`enforceBuildPathLegality(buildPath, items, ownedItemNames)` folds three rules into one first-entry-wins pass: no name twice in the path, no two different items sharing a mutex group, and no entry the shop would refuse given the player's CURRENT inventory (owned items seed the group-occupancy map before the sweep). Non-obvious findings:
...
- **Unknown owned names are inert by construction**: they only block an exact same-name re-buy (name equality needs no catalog proof) and can never occupy a group, so the sweep drops nothing it cannot prove illegal.
